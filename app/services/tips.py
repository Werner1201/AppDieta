def tips(summary: dict, water_ml: int) -> list[str]:
    ...
    if totals["protein"] < cfg["daily_protein"] * 0.6:
        out.append("Proteína baixa hoje. Um jantar com frango, ovos ou feijão ajuda.")
    if water_ml < cfg["daily_water_ml"] * 0.5:
        out.append("Água baixa. Comece com 250 ml agora.")
    if totals["fat"] > cfg["daily_fat"]:
        out.append("Gordura passou da meta. Prefira opções grelhadas no restante do dia.")
    if totals["carbs"] > cfg["daily_carbs"]:
        out.append("Carboidratos altos. Ajuste as próximas porções.")
